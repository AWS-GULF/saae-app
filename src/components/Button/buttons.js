...
export const RegularButtonWhiteBg = ({ href = "#", children }) => {
  ...
  return (
    <div className='flex justify-center items-center '>
      <Link
        href={href}
        className="cursor-pointer w-[235px] h-[64px] border-2 border-secondary rounded-2xl bg-white flex  gap-3 lg:gap-4 justify-center items-center"
      >
        <span className="text-xl font-[500] text-secondary flex justify-center items-center">{children || t('button_explore_more')}</span>
...
export const RegularButtonSecondaryBg = ({ href = "#", children }) => {
  ...
  return (
    <div className='flex justify-center items-center'>
      ...
      >
        <span className="text-xl font-[500] text-white flex justify-center items-center">{children || t('button_explore_more')}</span>
...
export const BackToHomeButton = ({ href = "#", children }) => {
  ...
  return (
    ...
    >
      <span className="text-xl font-[500] text-white justify-center items-center">{children || t('button_back_to_home')}</span>
      <Image src={arrowLight} alt="arrow right" className={locale === 'ar' ? 'rotate-180' : ''} />
    </Link>
  );
};
export const LargeButton = ({ href = "#", children, img }) => {
  ...
  return (
    ...
    >
      <span className="text-xl font-bold text-white justify-center items-center ">{children || t('button_request_consultation')}</span>
...
export const Submit = ({ children, disabled = false }) => {
  ...
  return (
    <button
      ...
      className={`${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} w-[185px] h-[45px] border-2 ${disabled ? 'bg-gray-300 border-gray-300' : 'bg-secondary border-secondary'} rounded-2xl flex gap-4 justify-center items-center`}
    >
      <span className="text-xl font-[500] text-white justify-center items-center">{children || t('button_back_to_home')}</span>
    </button>
  );
